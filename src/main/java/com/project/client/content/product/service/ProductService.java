...
public class ProductService implements ProductPort {
  private final RedisRepositoryPort redisRepositoryPort = new RedisRepository();
     ...
     */
    @Override
    public List<Product> getBySimilarity(String id) throws IOException, ExecutionException, InterruptedException {
    ...
    @Override
    public int getExFromRedis(String id) {
        return redisRepositoryPort.getExFromRedis(id);
    }


    @Override
    public void setExToRedis(String id, int exception) {
        redisRepositoryPort.setExToRedis(id, exception);
    }
}
